...
void TIM2_CH1_Init(){
    ...
    TIM2->PSC = 79999;       // Prescaler = 
    TIM2->ARR = 333;   // Auto-reload: Upcouting (0..ARR), Downcouting (ARR..0)
	  TIM2->CCR1  = 50;         // Output Compare Register for channel 1, 
    // select PWM mode 1 output on channel 1	(OC1M=110)
		TIM2->CCMR1 &= ~TIM_CCMR1_OC1M;  // Clear ouput compare mode bits for channel 1
    TIM2->CCMR1 |= TIM_CCMR1_OC1M_2 | TIM_CCMR1_OC1M_1; // set mode to 110
